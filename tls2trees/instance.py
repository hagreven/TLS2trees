...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n-zeros", default=3, type=int, help="leading zeros for tile names"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        nargs=2,
        type=float,
        help="boundary height for slice used for identifying stems: default [1.5, 2.]",
    )
    parser.add_argument(
    ...
    if params.tindex:
        ...
        for i, t in tqdm(
            enumerate(buffer_tiles.itertuples()),
    ...
    for i, b in tqdm(
        ...
        if params.save_diameter_class:
            ...
            ply_io.write_ply(
                os.path.join(params.odir, d_dir, f"{n}_T{I}.leafoff.ply"),
                trees.loc[trees.t_clstr == b],
            )
        else:
            ply_io.write_ply(
                os.path.join(params.odir, f"{n}_T{I}.leafoff.ply"),
                trees.loc[trees.t_clstr == b],
            )
        params.base_I[b] = I
        I += 1
    ...
    if params.add_leaves:
        ...
        branch_and_leaves = pd.concat(
            [cnrs, chull[["x", "y", "z", "label", "stem", "xlabel", "clstr"]]]
        )
