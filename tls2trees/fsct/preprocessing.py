...
def save_pts(params, I, bx, by, bz):
    pc = params.pc.loc[
        (params.pc.x.between(bx, bx + params.box_dims[0]))
...
def Preprocessing(params):
    ...
    if not isinstance(params.tile, int) and params.tile.isdigit():
    ...
    # read in pc
    params.pc, params.additional_headers = load_file(
        filename=params.point_cloud, additional_headers=True, verbose=params.verbose
    )
    ...
    if params.buffer > 0:
        ...
        distance, neighbours = nn.kneighbors(
            tile_index.loc[tile_index.fname == params.tile][["x", "y"]],
        ...
        buffer = pd.DataFrame()
        for tile in tqdm(
            tile_index.loc[neighbours[1:]].itertuples(),
        ...
        buffer = buffer.loc[
            (
                buffer.x.between(
                    params.pc.x.min() - params.buffer, params.pc.x.max() + params.buffer
                )
            )
            & (
                buffer.y.between(
                    params.pc.y.min() - params.buffer, params.pc.y.max() + params.buffer
                )
            )
        ]
    ...
    if params.subsample:  # subsample if specified
        if params.verbose:
            print("downsampling to: %s m" % params.subsampling_min_spacing)
        params.pc = downsample(
            params.pc, params.subsampling_min_spacing, accurate=False, keep_points=False
        )

    # apply global shift
    if params.verbose:
        print("global shift:", params.global_shift.values)
    ...
    if params.verbose:
        print("Preprocessing done in {} seconds\n".format(time.time() - start_time))
