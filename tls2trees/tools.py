...
def voxelise(tmp, length, method="random", z=True):
    ...
    if method == "random":
        code = lambda: "".join(
            np.random.choice([x for x in string.ascii_letters], size=8)
        )
...
def downsample(
    ...
    if accurate:
        ...
        pc.loc[:, "dist"] = np.linalg.norm(
            pc[["x", "y", "z"]].to_numpy(dtype=np.float32)
            - pc[["mx", "my", "mz"]].to_numpy(dtype=np.float32),
            axis=1,
        )
...
def make_dtm(params):
    ...
    X, Y = np.meshgrid(
        np.arange(
            params.pc.xx.min(),
            params.pc.xx.max() + params.grid_resolution,
            params.grid_resolution,
        ),
        np.arange(
            params.pc.yy.min(),
            params.pc.yy.max() + params.grid_resolution,
            params.grid_resolution,
        ),
    )

    ground_arr = pd.DataFrame(
        data=np.vstack([X.flatten(), Y.flatten()]).T, columns=["xx", "yy"]
    )
    ground_arr = pd.merge(
        ground_arr, VX_map, on=["xx", "yy"], how="outer"
    )  # map VX to ground_arr
    ground_arr = pd.merge(
        ground[["z", "VX"]], ground_arr, how="right", on=["VX"]
    )  # map z to ground_arr
    ...
    while np.any(np.isnan(ground_arr.ZZ)):
        ground_arr.loc[:, "ZZ"] = ndimage.generic_filter(
            ground_arr.z.values.reshape(*X.shape),  # create raster,
            lambda z: np.nanmedian(z),
            size=size,
        ).flatten()
        size += 2

    ground_arr[["xx", "yy", "ZZ"]].to_csv(
        os.path.join(params.odir, f"{params.basename}.dem.csv"), index=False
    )
...
def chunk_pc(pc, out_dir, params):
    def save_pts(pc, I, bx, by, bz, working_dir, params):
        ...
        if len(pc) > 1000:
            ...
            np.save(
                os.path.join(working_dir, f"{I:07}"),
                pc[["x", "y", "z", "label"]].values,
            )
    ...
    for i, (bx, by, bz) in enumerate(itertools.product(x_cnr, y_cnr, z_cnr)):
        threads.append(
            threading.Thread(target=save_pts, args=(pc, i, bx, by, bz, out_dir, params))
        )
