...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--point-cloud", "-p", default="", type=str, help="path to point cloud"
    )
    parser.add_argument(
        "--params", type=str, default="", help="path to pickled parameter file"
    )
    ...
    parser.add_argument(
        "--redo", type=int, default=None, help="which process to run to"
    )
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help='path to tile index in space delimited format "TILE X Y"',
    )
    parser.add_argument(
        "--buffer", default=0, type=float, help="included data from neighbouring tiles"
    )
    parser.add_argument(
        "--zeros", default=3, type=int, help="number of leading zeros in tile names"
    )
    ...
    parser.add_argument(
        ...
        default=10,
        type=int,
        help="If you get CUDA errors, try lowering this.",
    )
    parser.add_argument(
        "--num_procs",
        default=10,
        type=int,
        help="Number of CPU cores you want to use. If you run out of RAM, lower this.",
    )

    parser.add_argument(
        "--keep-npy",
        action="store_true",
        help="Keeps .npy files used for segmentation after inference is finished.",
    )
    ...
    parser.add_argument(
        "--model", default=None, type=str, help="path to candidate model"
    )
    ...
    if params.step >= 0 and not params.steps_completed[0]:
        ...
        pickle.dump(
            params,
            open(os.path.join(params.odir, f"{params.basename}.params.pickle"), "wb"),
        )
    ...
    if params.verbose:
        print(
            f"peak memory: {resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1e6}"
        )
